...
st.set_page_config(
    page_title="Ache Podcast Control Center",
    page_icon="🎙️",
    layout="wide",
...
# Título
st.markdown('<div class="main-header">🎙️ Ache Podcast Control Center</div>', unsafe_allow_html=True)

# Sidebar para acciones rápidas
with st.sidebar:
    base_dir = os.path.dirname(os.path.abspath(__file__))
    logo_path = os.path.join(base_dir, "assets", "logo_ache.png")
    ...
    if st.button("▶️ GENERAR PODCAST", type="primary"):
...
with tab7:
    ...
    if st.button("Generar Informe de Fuentes", type="primary"):
...
with tab8:
    st.markdown('<div class="sub-header">Análisis de Contenido (Nubes de Palabras)</div>', unsafe_allow_html=True)
    ...
    if st.button("Generar Nubes de Palabras", type="primary"):
        with st.spinner("Analizando contenido de noticias..."):
            ...
            if os.path.exists(cache_path):
                poblaciones, gal, temas = analizar_contenido_noticias(cache_path)
                
                if not poblaciones and not gal and not temas:
                    st.warning("No se encontraron datos suficientes o hubo un error al leer el cache.")
                else:
                    # Función auxiliar para pintar nube
                    def plot_cloud(counter, title, cmap='viridis'):
                        if not counter:
                    ...
                    with col_cloud1:
                        ...
                        plot_cloud(poblaciones, "Poblaciones Mencionadas", "plasma")
                        
                    with col_cloud2:
                        st.markdown("### 🚜 Grupos GAL")
                        plot_cloud(gal, "Grupos de Acción Local", "cividis")
                    
                    st.markdown("---")
                    st.markdown("### 🗣️ Temas y Eventos (General)")
                    plot_cloud(temas, "Palabras Clave", "viridis")
