...
import shutil
import pandas as pd
from src.utils.ui_common import inject_pcc_style, cargar_config, guardar_config, init_session_state, render_wavesurfer_player
                ...
                with col_exp:
                    with st.expander(f"🎙️ {display_name}"):
                        ...
                        if os.path.exists(cover_path):
                            col_img, col_info, col_act = st.columns([1, 2.5, 1])
                            with col_img:
                        ...
                        with col_info:
                            if mp3s:
                                render_wavesurfer_player(mp3s[0], key=f"pd_{p_dir}")
            ...
            for ee in ee_files:
                ...
                with col_exp:
                    with st.expander(f"📢 {os.path.basename(ee)}"):
                        ...
                        with col_a:
                            render_wavesurfer_player(ee, key=f"ee_{os.path.basename(ee)}")
                        with col_b:
            ...
            for od in od_files:
                ...
                with col_exp:
                    with st.expander(f"🎙️ {os.path.basename(od)}"):
                        col_a, col_b = st.columns([3, 1])
                        with col_a:
                            render_wavesurfer_player(od, key=f"od_{os.path.basename(od)}")
                        with col_b:
